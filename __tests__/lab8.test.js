describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$$('product-item');
    let prodShadowRoot = await prodItems[0].getProperty('shadowRoot');
    let prodButton = await prodShadowRoot.$('button');
    // console.log(prodButton.getProperty('innerText'));
    await prodButton.click();
    let buttonText = await prodButton.getProperty('innerText');
    let textValue = await buttonText.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let index = 1; index < prodItems.length; index++) {
      // console.log(`Clicking product item ${index + 1}/${prodItems.length}`);
    ...
    let textValue = await page.$eval('#cart-count', element => element.innerText);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for (let index = 0; index < prodItems.length; index++) {
      //console.log(`Checking button of product item ${index + 1}/${prodItems.length}`);
      let prodShadowRoot = await prodItems[index].getProperty('shadowRoot');
      let prodButton = await prodShadowRoot.$('button');
      let buttonText = await prodButton.getProperty('innerText');
      let textValue = await buttonText.jsonValue();
